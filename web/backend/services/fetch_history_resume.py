...
async def save_progress(done_items, remaining_items):
    progress = {
        'done': done_items,
...
async def main():
    ...
    done_urls = {item['url'] for item in progress.get('done', [])}
    remaining_links = [l for l in all_links if l['href'] not in done_urls]

    print(f'已完成: {len(done_urls)} 个')
    print(f'待抓取: {len(remaining_links)} 个')
    ...
    for link in remaining_links:
        ...
        if date_str and start_date <= datetime.strptime(date_str, '%Y-%m-%d') <= end_date:
    ...
    async with async_playwright() as p:
        ...
        for i, (date_str, period, url) in enumerate(tasks):
            print(f'[{i+1}/{len(tasks)}] {date_str} {period}...', end='', flush=True)
            ...
            try:
                ...
                if prices:
                    ...
                    if sheet_name:
                        ...
                        new_done.append({'date': date_str, 'period': period, 'url': url})
                        print(f' {len(prices)}条 ✓', flush=True)
                        success_count += 1
                    else:
                        print(' 保存失败', flush=True)
                        fail_count += 1
                else:
            ...
            # 每5个保存进度
            if (i + 1) % 5 == 0:
                await save_progress(progress.get('done', []) + new_done, tasks[i+1:])
                print(f'  进度已保存: 成功{success_count}, 失败{fail_count}')
                await asyncio.sleep(random_sleep(5, 10))

        await browser.close()

    # 最终保存进度
    await save_progress(progress.get('done', []) + new_done, [])
