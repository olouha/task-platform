...
class YantaiPriceScraper:
    ...
    async def recognize_captcha_from_url(self, img_url: str) -> str:
        """从URL识别验证码"""
        try:
            # 获取验证码图片
            response = await self.page.goto(img_url)
            ...
            if isinstance(result, str):
                ...
                code = result[0].strip() if result else ''
                print(f'[OCR] 识别结果: {code}')
                return code
            return ''
    ...
    async def recognize_base64_captcha(self, base64_data: str) -> str:
        """从base64数据识别验证码"""
        try:
            # 解码base64
    ...
    async def login_with_captcha(self):
        ...
        try:
            ...
            base64_match = re.search(base64_pattern, page_content)
            if base64_match:
                print('[INFO] 发现base64验证码')
                base64_data = base64_match.group(2)
                captcha_code = await self.recognize_base64_captcha(base64_data)
            else:
                # 查找URL验证码
                captcha_pattern = r'//mfs\.mysteelcdn\.com/[^"\'"]+\.png'
                url_match = re.search(captcha_pattern, page_content)
                if url_match:
                    captcha_src = 'https:' + url_match.group(0)
                    print(f'[INFO] 发现验证码URL: {captcha_src}')
                    captcha_code = await self.recognize_captcha_from_url(captcha_src)
        ...
        # 如果识别出验证码，填写它
        if captcha_code:
            print(f'[INFO] 填写验证码: {captcha_code}')
