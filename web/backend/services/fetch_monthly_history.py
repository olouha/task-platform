...
async def extract_date_from_url(url):
    """从URL提取日期和时段"""
    import re
...
async def main():
    print('=' * 60)
    print('山东烟台钢筋价格 - 历史数据抓取（直接点击日期）')
    print('=' * 60)
    print(f'抓取频率: 每月一次，停顿15秒')
    print()

    # 获取月份列表
    months = get_months()
    print(f'目标月份: {len(months)} 个')
    ...
    async with async_playwright() as p:
        ...
        while page_num <= 50:
            ...
            for link in links:
                date_str, period = await extract_date_from_url(link['href'])
                if date_str:
                    all_links.append({
                        'url': link['href'],
                        'date_str': date_str,
        ...
        print(f'总共找到 {len(all_links)} 条日期链接')

        # 按月份分组
        for year, month in sorted(months, reverse=True):
            print(f'\n--- 处理 {year}-{month:02d} ---')

            month_links = [l for l in all_links
                          if l['date_str'].startswith(f'{year}-{month:02d}')]

            if not month_links:
                print(f'  该月没有数据')
                continue

            print(f'  该月有 {len(month_links)} 条数据')

            # 过滤已存在的
            tasks = []
            for link in month_links:
                date_str = link['date_str']
                period = link['period']
                ...
                if sheet_name not in existing_sheets:
                    ...
                    print(f'  {date_str} {period} 已存在，跳过')
