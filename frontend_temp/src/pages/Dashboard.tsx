...
import api, { statsApi, yantaiRebarApi } from '../services/api'
...
export default function Dashboard() {
    ...
    const init = async () => {
      try {
        const [statsData, datesData] = await Promise.all([
          statsApi.get(),
          fetch(`${api.config.apiUrl}/api/yantai-db/dates`).then(r => r.json()).catch(() => ({ success: false, dates: [] }))
        ])
  ...
  const fetchCostData = async () => {
    ...
    try {
      // 先获取可用时期列表
      const response = await fetch(`${api.config.apiUrl}/api/cost-history/periods`).then(r => r.json())
      if (response && response.length > 0) {
        ...
        const concreteResponse = await fetch(
          `${api.config.apiUrl}/api/cost-history/concrete/by-period?year=${latestPeriod.year}&quarter=${encodeURIComponent(latestPeriod.quarter)}`
        ).then(r => r.json()).catch(() => null)

        // 获取钢筋数据
        const steelResponse = await fetch(
          `${api.config.apiUrl}/api/cost-history/steel/by-period?year=${latestPeriod.year}&quarter=${encodeURIComponent(latestPeriod.quarter)}`
        ).then(r => r.json()).catch(() => null)
  ...
  useEffect(() => {
    ...
    const fetchPrices = async () => {
      ...
      setDataError(null)
      try {
        const response = await fetch(`${api.config.apiUrl}/api/yantai-db/latest?date=${selectedDate}&limit=200`)
  ...
  useEffect(() => {
    ...
    const fetchComparison = async () => {
      try {
        const response = await fetch(`${api.config.apiUrl}/api/yantai-db/latest?date=${comparisonDate}&limit=200`)
        const data = await response.json()
        if (data.prices) {
